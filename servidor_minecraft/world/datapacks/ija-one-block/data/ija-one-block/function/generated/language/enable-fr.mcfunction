...
tag @e[tag=ija-a4-block] add ija-a4-lang-fr
...
scoreboard objectives modify ija-a4-usermined displayname {"text":"blocs"}
execute as @e[tag=ija-a4-upgrade-message] run data merge entity @s {"CustomName":"\"Amélioration dans\""}
execute as @e[scores={ija-a4-party-monster-time-left=1..}] run data merge entity @s {"CustomName":"{\"text\":\"Gardien des Monstres\",\"color\":\"dark_red\"}"}
